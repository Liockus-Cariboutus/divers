...
------ Movement ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
ENT.SightDistance = 3000 -- Initial sight distance | To retrieve: "self:GetMaxLookDistance()" | To change: "self:SetMaxLookDistance(distance)"
ENT.SightAngle = 177 -- Initial field of view | To retrieve: "self:GetFOV()" | To change: "self:SetFOV(degree)" | 360 = See all around
ENT.TurningSpeed = 45 -- [LDDN: 20] Initial turning speed | To retrieve: "self:GetMaxYawSpeed()" | To change: "self:SetMaxYawSpeed(speed)"
...
ENT.UsePoseParameterMovement = true -- [LDDN: false] Sets the model's "move_x" and "move_y" pose parameters while moving | Required for player models to move properly!
	-- ====== JUMPING ====== --
...
ENT.NextProcessTime = 0.1 -- MODIF 1 PAR DEFAUT LDDN -- Time until it runs the essential performance-heavy AI components
...
ENT.BecomeEnemyToPlayer = 2 -- Should it become enemy towards an allied player if it's damaged by them or it witnesses another ally killed by them?
...
ENT.Medic_SpawnPropOnHealModel = "models/weapons/tfa_codww2/usa_healthpack/w_usa_healthpack.mdl" -- The model that it spawns
...
ENT.DangerDetectionDistance = 800 -- [LDDN: 400] Max danger detection distance | WARNING: Most of the non-grenade dangers ignore this max value
...
ENT.FootstepSoundTimerRun = 0.25 -- Delay between footstep sounds while it is running | false = Disable while running
	-- ====== Idle Sound ====== --
ENT.HasIdleSounds = false -- Can it play idle sounds? | Controls "self.SoundTbl_Idle", "self.SoundTbl_IdleDialogue", "self.SoundTbl_CombatIdle"
...
-- [LDDN] :
function ENT:CustomOnAllyDeath(victim, attacker, inflictor) 
	self:FaceTowards(attacker:GetPos())            -- se tourner vers l’assaillant  
	self:SetEnemy(attacker)                        -- cibler le coupable  
  end
